...
> Named `wave-run` (not `wave`) because a command name must stay disjoint from the
...
Type `/wave-run` when you want to land a set of related changes via a safe parallel
...
For a **single-file change** with no parallelism: use `odoo-coder` or `odoo-code-reviewer` instead.
For a **requirement scoping / BRL** task: use `odoo-brl` instead.
For **in-context skill chaining** (no git): use `workflow-runner` instead.
...
```
/wave-run add wave orchestration skill + wiring + packaging (3 WI)
```
