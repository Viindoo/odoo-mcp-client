...
Thin dispatcher for the declarative upgrade-plan-full workflow.

**SSOT**: `plugins/odoo-semantic-skills/workflows/upgrade-plan-full.workflow.yaml`
...
Yield to the `workflow-runner` skill, passing this command's `$ARGUMENTS` (optional version
...
- Agent depth >= 1: this command dispatches to `workflow-runner` (depth 1); launch only from depth 0.

## See also

- `workflows/upgrade-plan-full.workflow.yaml` — phase contract (SSOT)
- `odoo-deploy-checklist` skill — use at actual deploy time
- `/odoo-bid-respond` — if this upgrade is part of a prospect proposal
