...
```
plugins/odoo-semantic-skills/workflows/feature-positioning.workflow.yaml
```

## How this command works

This command invokes the `workflow-runner` skill with the `feature-positioning` workflow.
...
Invoke this command and the `workflow-runner` will guide you through all phases with
...
- `plugins/odoo-semantic-skills/workflows/feature-positioning.workflow.yaml` — SSOT for phases, gates, model tiers
- `/odoo-content-draft` — full marketing pieces (blog, whitepaper, case study)
- `/odoo-bid-respond` — RFP positioning with pricing and timelines
- `/odoo-objection-handler` — reactive objection responses
