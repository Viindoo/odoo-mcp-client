...
- **brand_tokens_source**: .odoo-ai/brand-tokens.json  (optional — consumer-declared JSON map `token -> color`, e.g. `{"--primary": "#00BBCE"}`; enables the brand-fidelity checks in verify-frontend.sh Tier 4 + ui-reviewer Step 4b. Omit for pure-Odoo projects. No brand is vendored in the plugin.)
