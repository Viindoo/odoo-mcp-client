...
|---|---|---|
| `technical` | Module A must be installed before module B (Odoo manifest `depends`) | `module_inspect(method='dependencies')` (deterministic) |
...
D1. Technical bootstrap (deterministic, parallel MCP <=3):
    ...
    for module in modules:
        module_inspect(name=module, method='dependencies')   # -> {"depends": [...]}
