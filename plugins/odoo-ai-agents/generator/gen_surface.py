...
def gen_orchestration_map(orch: dict[str, dict]) -> str:
    ...
    lines.append("  natural-language dispatch; no subagent spawn of its own - see a skill's own spawns")
    lines.append("  cell for any anonymous review/resolver subagent) · `spawner-agent` (dispatches a")
    lines.append("  subagent).")
    lines.append("- **handoff** - Context-Handoff Protocol (CHP) tier for resuming subagents across turns.")
    lines.append("  `send-message` (Tier-A: a launcher resumes a child it launched, by the id its own")
    lines.append("  launch returned, avoiding cold-spawn overhead) · `fork` (Tier-B: subagent_type=fork fan-out inheriting parent")
    lines.append("  context + prompt cache) · `fresh` (Tier-C default: cold-spawn every turn via agent")
    lines.append("  launch + worklog blackboard - always-correct baseline; implicit when field is absent).")
