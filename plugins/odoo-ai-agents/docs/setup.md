...
| `odoo-forward-port` | Coder / Engineer | Continuous/one-shot Odoo forward-port (per-commit intent extract + absorb-all git topology: ONE merge of the range tip closed by ONE merge commit, keeping every source SHA; adaptive test forward); output under `<ISOLATE_DIR>/forward-port/`; invoke via `/odoo-forward-port` or plain-language intent |
