...
cmd_check() {
    ...
    # installed. Claude Code is always considered wired because the plugin
    # bundles its own .mcp.json. Codex and Gemini are OPTIONAL: a machine with
    # only Claude must not be reported as "needs wiring" just because
    # ~/.codex / ~/.gemini are absent. Their config file existing is the signal
