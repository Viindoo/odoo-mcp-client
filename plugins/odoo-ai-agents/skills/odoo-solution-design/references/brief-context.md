...
`odoo-test-writing` / `odoo-qa-suite`), and Risks - the architect's own Round 4 owns the doc
template. The Intent &
...
the architect's own - not restated here: its own Round 0-3 (grounding + override
