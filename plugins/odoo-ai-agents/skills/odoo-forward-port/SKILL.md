...
merge (P11), human-confirmed.

**WORK tier (when a phase fans out):** from the integration worktree, each parallel unit
(one module or work-item in P8 adapt) gets its OWN `git worktree add` child worktree +
...
at P2; a `status=adapted` commit resumes at P9). Enumerate commits with
...
model tier INLINE (`git show --stat` + one `find_override_point` probe - the orchestrator
...
grounds categories 1-2, so a probe there is wasted. Dispatcher inputs (canonical contract, pass
exactly): `{ module, repo_root, source_ref, target_ref, target_version }`. `repo_root` is the
MAIN checkout root where git runs - the integration worktree does NOT exist at P2 (it is created
at P4); never reference it here. `source_ref` / `target_ref` are the source / target git refs.
...
After Plan Mode approval, create the JOB-tier integration worktree from B (Hard rule 1):
`git worktree add -b fp/<slug> <path>/fp-integration <target-branch>`. No branch is created
before this point - everything up to and including the plan gate is read-only.
...
**P5 - Git merge --no-commit [critical section, in integration].** Continuous:
...
absorption zone (P6 -> P8 -> P9 all happen before the commit). SSOT: `[[fp-merge-absorption]]`.
