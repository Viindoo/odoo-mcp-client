...
3. **Translate API** to `tgt_version` - framework imports, Form helper path, `@tagged`
   ...
   targeting `tgt_version`. Specifically: call `test_base_classes(odoo_version='<tgt_version>')` to confirm the correct base class for the target - `SavepointCase` is a DISTINCT class on v12-v15 and a DEPRECATED ALIAS of `TransactionCase` from v16+; adapt to `TransactionCase` only when `tgt_version` is v16 or later (keep `SavepointCase` on v12-v15 targets). NOTE: `test_base_classes` currently mislabels this alias window as `v8-v15` (a known server-side annotation bug); treat the annotation string as advisory for this one symbol and apply this rule instead - see `${CLAUDE_PLUGIN_ROOT}/snippets/odoo-era-boundaries.md`. Also reaffirm the `cr.commit()` FORBIDDEN contract from the same call. Call `tests_covering(model='<model>', odoo_version='<tgt_version>')` to check whether an equivalent test already exists on the target - if it does, record as outcome (a) "already covered on target" and skip forward-port of that method (see [[fp-merge-absorption]]).
