...
- **Test-first (before the code).** Author a module's failing test BEFORE the implementation - independent from the coder - so the test specifies intended behavior and the code is written to green. Used standalone, or when a caller explicitly hands off red-first authoring. (The `odoo-coding` loop authors its OWN red test inline via a separate coder in TEST-AUTHOR MODE, so it does NOT route here.)
