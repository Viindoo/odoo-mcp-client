...
3. **Translate API** to `tgt_version` - framework imports, Form helper path, `@tagged`
   ...
   targeting `tgt_version`. Specifically: call `test_base_classes(odoo_version='<tgt_version>')` to confirm the correct base class for the target - `TransactionCase` is the base class on v8-v11 (`SavepointCase` does not exist in that range - era-boundaries row 3); `SavepointCase` is a DISTINCT class on v12-v14; at v15 `TransactionCase` absorbed the class-level savepoint/`setUpClass` behavior and `SavepointCase` became a deprecated alias (Odoo core migrated at v15). Adapt to `TransactionCase` when `tgt_version` is **>= v15**; keep `SavepointCase` on v12-v14 targets. NOTE: `test_base_classes` per-class version tags are STATIC / version-invariant (it returns `(deprecated alias, v8-v15)` for SavepointCase at every version - a known server-side annotation bug); the authoritative absorb/adapt boundary is v15 - verify any version-sensitive class claim against the usage graph (`test_class_inspect(..., method='hierarchy')`) instead of the static tag - see `${CLAUDE_PLUGIN_ROOT}/snippets/odoo-era-boundaries.md`. Also reaffirm the `cr.commit()` FORBIDDEN contract from the same call. Call `tests_covering(model='<model>', odoo_version='<tgt_version>')` to check whether an equivalent test already exists on the target - if it does, record as outcome (a) "already covered on target" and skip forward-port of that method (see [[fp-merge-absorption]]).
