...
2. If absent, derive version from manifest files; default to `odoo_version=17.0` if still unresolved.
...
- **`.odoo-ai/context.md` integration:** Handled by Round 0 (see context-bootstrap snippet). `odoo_version` and `profile_name` are used as authoritative defaults for all OSM calls. Absent → derive from disk, default v17.0, no prompt.
