...
1. **On a CRITICAL or HIGH finding that needs a code change, drive the fix yourself - attempt CHP Tier A first, fall back to Tier C.** Per `${CLAUDE_PLUGIN_ROOT}/snippets/context-handoff-protocol.md` Tier A, BOTH conditions must hold: plan.md records the id an earlier launch in THIS context returned for that module's coder, and a messaging tool is in your current toolset. Then take the **Tier-A path** - send the review findings + fix instructions (framed as shared team context / prior-phase findings, never as "secret") to that recorded id, then **PARK: end your turn here (emit nothing after the send), do NOT await a synchronous return - YOU are woken when the worker completes, whatever your own depth.** Either condition false - no id recorded, the id no longer resolves, or no messaging tool - take the **Tier-C fallback** (always correct): **IMMEDIATELY invoke `odoo-coding` via the Skill tool yourself**, passing it the review report path, the exact findings to fix, and the literal line **"AUTONOMOUS FIX (review-driven): skip your Phase 0 human gate, fix to these findings, then invoke odoo-code-review to verify"** (keep this sentinel text unchanged for the Tier-C path). The park works at any depth, but ONLY if you actually stop: send, then end the turn and emit nothing after it - a turn that sends and keeps working leaves no point at which the worker's result can be handed back (R0 move 3 / R1 § Boundary, `${CLAUDE_PLUGIN_ROOT}/snippets/spawner-completion-contract.md`). On either path this skill reviews, it does not author: the fix is written by `odoo-coding`, never here. Either way this is autonomous - do NOT pause for a human (running the review IS the opt-in).
2. The coder fixes, then the loop comes back to you to verify - on the Tier-A path you are woken when the worker completes and you read its returned result; on Tier-C the `odoo-coding` invocation returns to you inline. That round-trip - **review → code → review** - repeats until the review is clean (no CRITICAL/HIGH and the behavior is covered).
