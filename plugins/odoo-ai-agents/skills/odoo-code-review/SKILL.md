...
1. **On a CRITICAL or HIGH finding that needs a code change, drive the fix yourself - attempt CHP Tier A first, fall back to Tier C.** Per `${CLAUDE_PLUGIN_ROOT}/snippets/context-handoff-protocol.md` Tier A, all THREE conditions must hold: plan.md records the id an earlier launch in THIS context returned for that module's coder, a messaging tool is in your current toolset, AND you are running in the ROOT conversation. Then take the **Tier-A path** - send the review findings + fix instructions (framed as shared team context / prior-phase findings, never as "secret") to that recorded id, then **PARK: end your turn here (emit nothing after the send), do NOT await a synchronous return - the root is resumed when the worker completes.** ANY condition false - no id recorded, the id no longer resolves, no messaging tool, or you were dispatched into a context below the root - take the **Tier-C fallback** (always correct): **IMMEDIATELY invoke `odoo-coding` via the Skill tool yourself**, passing it the review report path, the exact findings to fix, and the literal line **"AUTONOMOUS FIX (review-driven): skip your Phase 0 human gate, fix to these findings, then invoke odoo-code-review to verify"** (keep this sentinel text unchanged for the Tier-C path). Below the root the park is not a slower option, it is a dead end - nothing wakes a dispatched context, so a send-and-stop there ends the run silently (R0 move 2 / R1 § Boundary, `${CLAUDE_PLUGIN_ROOT}/snippets/spawner-completion-contract.md`). Either way this is autonomous - do NOT pause for a human (running the review IS the opt-in).
2. The coder fixes, then the loop comes back to you to verify - on the Tier-A path the root is resumed when the worker completes and you read its returned result; on Tier-C the `odoo-coding` invocation returns to you inline. That round-trip - **review → code → review** - repeats until the review is clean (no CRITICAL/HIGH and the behavior is covered).
