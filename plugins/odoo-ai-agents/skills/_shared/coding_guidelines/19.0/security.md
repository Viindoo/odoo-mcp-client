> Source: official Odoo 19.0 security documentation - https://raw.githubusercontent.com/odoo/documentation/19.0/content/developer/reference/backend/security.rst
...
Any public method can be executed via an RPC call with the chosen parameters. The methods
...
# nearly there (corrected from upstream: x[x] -> x[0], added the missing closing bracket)
auction_lots_ids = [x[0] for x in self.env.execute_query(SQL("""
...
Even better, there exists an `odoo.tools.SQL` wrapper to build queries
using templates that handles the formatting of inputs.
See the ORM SQL execution documentation for detailed usage.
...
online documentation of psycopg2 to learn how to use it properly:
...
You would be tempted to manipulate these lists directly, however it can
introduce subtle issues where the user could inject a domain if the input is
not normalized.
Use `Domain` (from `odoo.fields`) to handle safely the manipulation
of domains.
...
`str` operations to *automatically escape parameters*.
...
The `escape` method (and its alias `odoo.tools.misc.html_escape`) turns a `str` into
...
from the text. The translation methods accept a `Markup`
parameter and will escape the translation if it receives at least one.
...
guaranteed to be safe or trusted), it is still required (e.g. to avoid breaking
...
**Sanitizing** converts *CODE* to *SAFER CODE* (but not necessarily *safe* code).
...
Some may want to use `eval` to parse user provided content. Using `eval` should
be avoided at all cost. A safer, sandboxed, method `odoo.tools.safe_eval`
...
If the values of a record need to be retrieved or modified dynamically, one may
