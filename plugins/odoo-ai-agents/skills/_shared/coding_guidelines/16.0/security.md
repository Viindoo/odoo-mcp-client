> Source: official Odoo 16.0 security documentation - https://raw.githubusercontent.com/odoo/documentation/16.0/content/developer/reference/backend/security.rst
...
Any public method can be executed via a RPC call with the chosen parameters. The
methods starting with a `_` are not callable from an action button or external API.
...
```pycon
>>> code = "<p class='text-warning'>Important Information</p>"  # corrected from upstream typo
