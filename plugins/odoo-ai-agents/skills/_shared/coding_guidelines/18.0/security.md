> Source: official Odoo 18.0 security documentation - https://raw.githubusercontent.com/odoo/documentation/18.0/content/developer/reference/backend/security.rst
...
`Markup` is that it's a very rich type overriding  <!-- corrected from upstream typo: overrinding -->
...
> **Tip:** Most of the content-safe APIs actually return a `Markup` object with all that implies.

The `escape` method (and its alias `html_escape` from `odoo.tools.misc`) turns a `str` into
...
parameter and will escape the translation if they receive at least one.
...
> **Important:** Escaping is always 100% mandatory when you mix data and code, no matter how
> safe the data.
...
be avoided at all cost. A safer, sandboxed method `safe_eval` from `odoo.tools`
