...
Within each of these three groups, the imported lines are alphabetically sorted.
...
- Collections are booleans too : In python, many objects have "truthness" in boolean context. For example, all collections (lists, dicts, sets, ...) are `False` when empty:
...
So, you may write `if some_collection:` instead of `if len(some_collection):`.
...
The context is a frozen dictionary that cannot be modified. To call a method with a different context, the `with_context` method should be used :
...
The last example is the most efficient because it allows submodules to extend or modify the behaviour easily. Naming your functions correctly is the basis of a good code: it eases the reading flow and code understanding. Naming small functions is also a good documentation.

### Never commit the transaction

The Odoo / OpenERP framework is in charge of providing the transactional context for all RPC calls. The principle is that a new database cursor is opened at the beginning of each RPC call, and committed when the call has returned, just before transmitting the answer to the RPC client, approximately like this:
...
- etc. (see general rule above if you really think you have an exception)
