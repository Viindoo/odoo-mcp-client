...
If for some reason you need to bypass the pre-commit hook, for example because you're working on something which is not valid yet, but want to make a temporary commit to switch branches, you may do so by bypassing the pre-commit hook (pass the no-verify flag when committing). Amending this commit at a later point will run the pre-commit again.
