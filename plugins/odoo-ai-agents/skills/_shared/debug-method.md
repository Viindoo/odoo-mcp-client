...
4. **Bisect - halve the search space.** The fault lies between "data still correct here" and
   ...
   remains. To isolate a regressing commit over time, invoke the `git-toolkit:git-ops` skill to run a bisect in a dedicated worktree; binary-search the call stack / data flow
   for a logic fault.
