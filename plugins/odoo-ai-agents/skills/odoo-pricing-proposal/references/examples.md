...
- Per-user: CE self-hosted has no Odoo SA license fee; partner support fee applies
- Implementation: 15 days × `<SME day rate>` = `<total>` (AE to fill)
- Support: Basic tier, `<basic rate>` / year
- Payment: 30/40/30 milestone
- No discount applied
- Next step: Review proposal in a 30-minute call; target SOW signature within 2 weeks
