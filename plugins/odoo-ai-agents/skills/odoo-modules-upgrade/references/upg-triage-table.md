...
| 3 | Action=DELETE-absorbed (the coder runs `git rm` + dep cleanup only - no business logic written); OR action=KEEP with only string/label/path fixes (no ORM, no view restructure); OR a single manifest version bump with no logic change | **haiku** |
...
- **DELETE-absorbed modules always use haiku** (the work is `git rm` + dep list edits;
  no business logic is written; haiku is sufficient).
