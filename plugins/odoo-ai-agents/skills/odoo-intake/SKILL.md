...
2. **No `writes-files` specialist before Plan Mode is approved.** Three points, none optional:
   - (a) Never run `odoo-coding`, `odoo-brl`, `workflow-chaining`, or any `output_mode = writes-files` skill before approval. Before approval, only describe it in the Proposed Plan. (The per-wave coding integration is owned by `run-harness` itself via its between-wave integration, driven from the approved plan - there is no user-invocable git-executor skill.)
...
6. **Worktree isolation - universal git-safety default (no exceptions).** Before ANY dispatched skill/workflow writes to a git-tracked file (create/modify/delete) or commits, a dedicated worktree/branch MUST be provisioned via `git-toolkit:git-ops` and the write MUST happen there - never in the principal checkout (a direct mutation corrupts any other session sharing the branch). This POLICY applies on EVERY path out of intake (Tier-1/3 fast-path, Tier-4 brainstorm, the trivial inline-micro-plan case, the Plan-Mode-exempt fast-paths). ENFORCEMENT: every `writes-files` node engages Phase P (engage condition #2) and is provisioned by `run-harness` at dispatch (its Hard rule 6); intake provisions directly only for a `writes-files` skill it Skill-tool-dispatches WITHOUT a run file. Self-provisioning specialists (SSOT set: `${CLAUDE_PLUGIN_ROOT}/snippets/git-delegation.md` § Self-provisioning specialists) isolate internally on every path; `odoo-coding` is a self-provisioning fallback if it receives none. No `writes-files` dispatch ever targets the principal checkout. **Exempt:** read-only work (recon, review of `TARGET=local`, brainstorming) and any deliverable confined to `.odoo-ai/` or `/tmp`. Mechanics: `${CLAUDE_PLUGIN_ROOT}/snippets/git-delegation.md` (SSOT - S9 worktree-always invariant; do not restate here).
...
**Why intake can do this**: `EnterPlanMode`/`ExitPlanMode` are main-context harness tools; intake initiates. Semantics: `${CLAUDE_PLUGIN_ROOT}/snippets/planning-gate-contract.md` § Plan-Mode enter/exit + plan_mode_active.

**Does NOT apply** for: `odoo-feature-check`, `odoo-version-diff`, `odoo-risk-overview`, `odoo-deprecation-audit`, `odoo-gap-analysis`, `odoo-discovery-summary`, `odoo-capability-proof`, `odoo-objection-handling`, `odoo-content-draft`, `odoo-competitive-brief`, `odoo-solution-design`, `odoo-planning`, any `chat-only` skill. (`odoo-solution-design` and `odoo-planning` write only `.odoo-ai/`, and `odoo-planning` owns its own Plan Mode via `plan_mode_active` - intake does not wrap them in a second Plan Mode.) Also NOT: `odoo-deep-survey`, `odoo-code-review`, `odoo-debug`, `odoo-forward-port`, `odoo-git-rebase`, `odoo-modules-upgrade` (skip intake's Plan Mode by design - each uses the shared Plan-Mode gate itself).
...
6. **Worktree isolation (Hard rule 6, no exceptions).** If the Approach is a self-provisioning specialist (SSOT set: `${CLAUDE_PLUGIN_ROOT}/snippets/git-delegation.md` § Self-provisioning specialists), skip this step - it provisions internally. On the common path a `writes-files` Approach engages Phase P and is provisioned by `run-harness` at dispatch (its Hard rule 6), NOT here. Only in the non-Phase-P case, BEFORE dispatch, invoke `git-toolkit:git-ops` via the Skill tool to create a dedicated worktree/branch and pass its path into the dispatched skill's brief as its working root (e.g. `WORKTREE_PATH` for `odoo-coding`, `TARGET: worktree:<path>` otherwise). The principal checkout is NEVER the target. Mechanics: `${CLAUDE_PLUGIN_ROOT}/snippets/git-delegation.md`.
7. ONLY after Plan Mode approval AND worktree provisioning (inline per step 6 in the rare non-Phase-P case, or via Phase P -> `run-harness` Hard rule 6 in the common case), the dispatcher invokes the execute-skill via the **Skill tool** (`run-harness` in the common case, else the main agent directly; a skill is not an agentType - Agent-tool'ing a skill name fails; see § Dispatch mechanism).
...
The implementation plan written inside Plan Mode (step 3 above) is a 3-block `writes-files` plan whose schema is SSOT at `${CLAUDE_PLUGIN_ROOT}/skills/odoo-intake/references/plan-mode-schema.md` (block schemas incl. the REQUIRED module-DAG ASCII dep-graph, worked examples, rejection flow). **ALWAYS delegate 3-block authoring to `odoo-planning`** (its `odoo-planner` produces the wave-batched module-DAG + integration cadence + lifecycle wiring); intake NEVER writes the plan inline - planning is mandatory for all work (`${CLAUDE_PLUGIN_ROOT}/snippets/planning-gate-contract.md` § Mandatory-planning rule). **When delegating, pass `plan_mode_active: true` in the `odoo-planning` dispatch brief** - Plan Mode was already entered at step 2, so `odoo-planning` MUST NOT call `EnterPlanMode` again (double-enter harness error); the flag tells its Plan Mode guard to skip enter.
