...
| 11 | `odoo-git-rebase` vs `odoo-forward-port` vs parallel multi-module | same series + one branch's whole range = git-rebase; cross-major single commit = forward-port; N disjoint modules squashed = `odoo-planning` (plans it; `run-harness` drives it node by node via its single node loop) |
