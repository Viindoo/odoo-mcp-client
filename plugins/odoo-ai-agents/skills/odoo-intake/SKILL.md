...
| 53 | "đăng module lên Apps Store", "đóng gói tài liệu module", "module packaging", "store listing", "App Store submission bundle", "chuẩn bị module lên marketplace" | `module-packaging` (workflow) | Full packaging bundle: icon-design + doc-illustration marketing landing + user-guide + manifest audit BEFORE App Store submission (vs `odoo-doc-illustration` for screenshot docs only; vs `odoo-icon-design` for icon only; vs `odoo-doc-feature-map` for feature inventory only) |
