...
2. **No `writes-files` specialist before Plan Mode is approved.** Three points, none optional:
   - (a) Never run `odoo-coding`, `odoo-brl`, `workflow-chaining`, or any `output_mode = writes-files` skill before approval. Before approval, only describe it in the Proposed Plan. (The `odoo-wave` git-executor is `user-invocable: false` - intake never dispatches it; `run-harness` does, from the approved plan.)
   - (b) Phase R MAY launch a READ-ONLY recon subagent (`Explore`, or an anonymous recon agent) to survey current state; a read-only **leaf skill** (e.g. `odoo-feature-check`, `odoo-override-finding`) is instead invoked via the **Skill tool** (a skill name is not an agentType). The recon agent MUST NOT write any file and MUST NOT spawn further (see `${CLAUDE_PLUGIN_ROOT}/snippets/worker-brief.md`). **Recon briefs MUST NOT instruct a subagent to WRITE or persist a file** - read-only recon agent types may lack the Write tool, so the agent dumps its output into chat instead. If recon findings must persist, the PARENT skill writes them to `.odoo-ai/<subdir>/<findings>.md` AFTER the recon agent returns. Read-only OSM calls (`model_inspect`, `check_module_exists`, `find_override_point`, `impact_analysis`) are likewise allowed.
...
6. **Worktree isolation - universal git-safety default (no exceptions).** Before ANY dispatched skill/workflow writes to a git-tracked file (create, modify, delete) or commits, a dedicated worktree/branch MUST be provisioned via `git-toolkit:git-ops` and the write MUST happen there - never in the principal checkout. Why: other sessions may share the principal branch, and a direct mutation on it corrupts their working tree. The worktree-isolation POLICY applies on EVERY path out of intake (Tier-1/3 fast-path, Tier-4 brainstorm, the trivial inline-micro-plan case, and the Plan-Mode-exempt fast-paths). ENFORCEMENT: every `writes-files` node engages Phase P (engage condition #2) and is therefore provisioned by `run-harness` at dispatch (its Hard rule 6); intake provisions directly ONLY for a `writes-files` skill it Skill-tool-dispatches WITHOUT a run file (none, given the engage rule - retained for completeness). Self-provisioning specialists (SSOT set: `${CLAUDE_PLUGIN_ROOT}/snippets/git-delegation.md` § Self-provisioning specialists) isolate internally on every path. Either way no `writes-files` dispatch ever targets the principal checkout; `odoo-coding` is a self-provisioning fallback if it somehow receives none. **Exempt:** read-only work (recon, review of `TARGET=local`, brainstorming) and any deliverable confined to `.odoo-ai/` or `/tmp`. Mechanics + invocation contract: `${CLAUDE_PLUGIN_ROOT}/snippets/git-delegation.md` (SSOT - S9 worktree-always invariant; do not restate its mechanics here).
...
6. **Worktree isolation (Hard rule 6, no exceptions).** If the Approach is one of the self-provisioning specialists (SSOT set: `${CLAUDE_PLUGIN_ROOT}/snippets/git-delegation.md` § Self-provisioning specialists), skip this step - it provisions its own worktree/branch internally. For the common path, a `writes-files` Approach engages Phase P and is provisioned by `run-harness` at dispatch (its Hard rule 6) - NOT here; this direct-branch provisioning is the non-Phase-P case only. In that non-Phase-P case, BEFORE dispatch, invoke `git-toolkit:git-ops` via the Skill tool to create a dedicated worktree/branch, and pass its path into the dispatched skill's brief as its working root (e.g. `WORKTREE_PATH` for `odoo-coding`, `TARGET: worktree:<path>` for skills that define that input). The principal checkout is NEVER the target of this dispatch. Mechanics + invocation contract: `${CLAUDE_PLUGIN_ROOT}/snippets/git-delegation.md`.
7. ONLY after Plan Mode approval AND worktree provisioning has occurred - either inline here per step 6 (rare non-Phase-P case) or, in the common case, via Phase P -> `run-harness` Hard rule 6 - the dispatcher invokes the execute-skill via the **Skill tool** (`run-harness` in the common case, or the main agent directly otherwise; a skill is not an agentType - Agent-tool'ing a skill name fails; see § Dispatch mechanism).
...
The implementation plan written inside Plan Mode (step 3 above) MUST contain three blocks, none optional for a `writes-files` Approach: **Block 1 - Workitem list** (each WI: `id`, one-line description, disjoint `files-in-scope`); **Block 2 - Dependency graph** (DAG with typed edges + topological order, or one of the four wave topologies for a few WIs); **Block 3 - Assignment** (`WI → skill|command|agent` + `effort` + `est_agents` ESTIMATE + per-WI acceptance criteria + verify command; the plan binds WHICH skill, never a per-agent `model` or fan-out `count` - the dispatched specialist skill owns those at runtime, so estimates are ADVISORY / du kien, non-binding). A workflow-command is ONE WI (its `output_dir/`), never expanded into its internal phases. **Authoring split (step 3):** for a NON-TRIVIAL multi-module change, DELEGATE authoring of this 3-block plan to `odoo-planning` (its `odoo-planner` agent produces the wave-batched module-DAG + integration cadence + lifecycle wiring); write the inline light micro-plan yourself ONLY for the trivial single-WI case. Either way the plan CONFORMS to the same schema. **When delegating, pass `plan_mode_active: true` in the `odoo-planning` dispatch brief** - Plan Mode was already entered at step 2, so `odoo-planning` MUST NOT call `EnterPlanMode` again (it would be a double-enter harness error); the flag tells its Plan Mode guard to skip enter (see `odoo-planning` § Plan Mode guard). **When writing a writes-files plan, read `${CLAUDE_PLUGIN_ROOT}/skills/odoo-intake/references/plan-mode-schema.md`** for the full block schemas, worked examples, and the rejection flow.

A `writes-files` inline micro-plan is `[code, review, integrate]`. `integrate` is the SAME land tail the full lifecycle and `odoo-wave` use: after review is clean, `run-harness` invokes `git-toolkit:git-ops` to push the WI branch and open a PR against the principal branch, then emits a Continuation-Contract `next` -> `odoo-pr-monitoring` at `gate_tier: L2` (the single outward merge gate). No squash machinery is needed for one reviewed commit. This is the ONE land mechanism (git-ops open-PR -> `odoo-pr-monitoring` merge); there is no local merge to the principal.
...
|---|---|---|
| a **skill** (`leaf` or `spawner-agent`/`spawner-wave`) - e.g. `odoo-code-review`, `odoo-coding`, `odoo-feature-check`, `odoo-planning` | a **skill**, NOT an agentType | **Skill tool** (deterministic). For a `spawner-agent` skill the Skill tool loads it in the main context so the skill itself fans out its own subagents. |
...
| **3 - keyword table** | 49-row routing table (see § Routing Table) covering all 9 persona domains | 0 | Map to single skill or workflow → soft-plan-gate |
...
Assignment (skill/agent + effort + est_agents): <WI → skill|agent (effort S/M/L/XL, est_agents N - ADVISORY/du kien; model + count owned by the dispatched skill at runtime, never bound here)>
...
7. Hard rule 6 (worktree isolation) → before the writes-files dispatch in step 4, a dedicated worktree/branch MUST be provisioned - in the common case via Phase P -> `run-harness` Hard rule 6 at dispatch, and inline per § Plan Mode Procedure step 6 only in the rare non-Phase-P case; the principal checkout is never the target, on any path including the ambiguous/inline-micro-plan one.
...
| 40 | "do this as a wave", "parallelize these changes", "multi-WI PR with review and squash", "land N related changes safely without touching main", "split this work into parallel worktrees", VI: "chia viec song song", "chay nhu mot wave" | `odoo-planning` | User-facing parallel / multi-WI delivery routes to the PLANNER, never to an executor: `odoo-planning` produces the wave-batched plan (design-first chain `odoo-solution-design -> odoo-planning`); on approval `run-harness` dispatches the INTERNAL `odoo-wave` git-executor (integration branch + per-WI worktrees + cherry-pick + end-of-wave review + 1 PR + squash, then STOPS at the L2-squash-gate - it never merges; the merge is owned by the subsequent `odoo-pr-monitoring` at the L2-merge-gate). `odoo-wave` is `user-invocable: false` - NEVER route a user prompt to it. (vs `odoo-coding` = a SINGLE change, no git orchestration; vs `odoo-brl` = classify/cost requirements, writes NO code) |
...
| 45 | "rebase onto", "rebase branch onto another branch same version", "absorb my branch onto an updated base", "rebase feature onto X", "rebase intent", VI: "rebase nhánh lên nhánh khác cùng phiên bản", "đưa nhánh feature lên base mới", "gộp ý đồ khi rebase nhánh" | `odoo-git-rebase` | SAME Odoo series; whole-range `rebase --onto` absorbing intent (vs `odoo-forward-port` = CROSS major; vs parallel N-disjoint-WI delivery = `odoo-planning`, which plans it for the internal `odoo-wave` executor; vs `odoo-coding` = one change, nothing to replay) |
...
| 9 | parallel multi-WI vs `odoo-brl` vs `odoo-coding` | parallelize+PR+squash → `odoo-planning` (plans it; `run-harness` dispatches the internal `odoo-wave`); classify/cost reqs → brl; single change → coding |
...
| 11 | `odoo-git-rebase` vs `odoo-forward-port` vs parallel multi-WI | same series + one branch's whole range = git-rebase; cross-major single commit = forward-port; N disjoint WIs squashed = `odoo-planning` (plans it; `run-harness` dispatches the internal `odoo-wave`) |
