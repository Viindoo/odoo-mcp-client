...
- `test_coverage_audit(module='<module>', odoo_version='<version>')` - surfaces uncovered
  fields and methods; use this to bound the scope (test only the gaps).
...
COVERAGE GAPS: <fields/methods flagged by test_coverage_audit(module='<module>', odoo_version='<version>')
  as uncovered - prioritise these gaps in the new tests>
