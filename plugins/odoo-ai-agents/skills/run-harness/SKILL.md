...
2. **Never hard-block the main agent - but that is not an open license to pause.** This loop is
   ...
   `enforce-grounding`.) That mechanism fact does NOT make "a node just finished" a reason to
   ...
   stop/abort phrase (§ Circuit-breakers below). Finishing a node, or any single subagent
...
5. **L2 is always a human gate.** The autonomy dial can lower L1→auto-pass but can NEVER lower
   ...
   party). The ONLY coding-run L2 is the outward MERGE (owned by `odoo-pr-monitoring`). No local
   merge into the principal checkout; no auto-merge. EVERY node's tier, `integrate` included, comes
   from the ONE total function in § Gate-tier resolution - never from a prose exception here.
...
- `repos[]` from that file: one entry per repository the run touches, each carrying that repo's
  ...
  run-integration.md § Repo Capability Card Template; § Run start step 2 re-derives it).
- Each node's `repo` names one of those `id`s. **`repo: null` is legal ONLY for a node that writes
  ...
  routing / report node (rule owner: harness §8.3 § `repo: null` legality). Every source-writing,
  `integrate`, and lifecycle node (verify, review, i18n, acceptance, doc, monitor, merge) MUST name
  a declared `id`. § Plan agreement check 2 proves this before every dispatch; never let an illegal
  one pass as merely "outside every repo's scope".
...
2. **Then fork, ONE branch + worktree pair PER ENTRY in `RUN.repos[]`** (a one-entry `repos[]` makes
   this exactly one pair): invoke `git-toolkit:git-ops` to add worktree
...
**The three lineage invariants - hold them, never re-derive them per node** (mechanics:
`run-integration.md` § Run start procedure).

1. ONE run-integration branch per repo, forked ONCE at step 2. No per-node, per-level or per-stage
   integration branch, and it is never re-forked mid-run.
2. Every source-writing node's worktree forks THAT branch - of the node's OWN `repo`, never
   `base`/principal and never another repo's. It already carries every prior node's commit, so a
   dependent node's worktree CONTAINS its dependencies' source; putting that source on a
...
**Single-unit collapse.** COUNT the source-writing nodes whose `repo == R`; never infer the collapse
from an absent field. At exactly ONE, dispatch it straight into that repo's `run-integration`
worktree and let it commit there - no child worktree, no cherry-pick, no per-node checkpoint. Rule +
the reason `n >= 2` keeps the child worktree: `run-integration.md` § Single-unit collapse (ONE owner).
...
loop:
    ...
    if RUN.schema_version predates run/2.0:   # serialized before this release; corroborate on the node
                                              # objects (a retired `approach_kind` value, or a
                                              # `nodes[].gate_tier`/`topology`/`cumulative_modules` field).
        set RUN.status = "NEEDS_CONTEXT"      # Do NOT drive it; do NOT guess a translation.
        blocked_reason = "run file uses the retired schema - re-plan via odoo-planning"
    ...
    node = pick_ready(RUN)        # READY = every depends_on is DONE; topo-order; tie -> lowest node id
                                  ...
                                  # `integrate@R` carries an EXTRA readiness predicate - evaluate it
                                  # HERE, scoped by node.repo (§ integrate readiness)
    ...
    verify_plan_agreement(node)   # FIVE checks (§ Plan agreement below): each AGREES, or STOPS the run
                                  # BLOCKED and routes back to odoo-planning.
    ...
    provision_worktree_if_needed(node)   # Hard rule 6 + Run start invariant 2: SOURCE-writing, no worktree,
                                         # not self-provisioning -> git-ops forks one FROM this repo's
    ...
    dispatch(node):               # ONE node per iteration; this loop NEVER has two dispatches in flight.
                                  # Every branch below is a synchronous call in THIS context, and nothing
                                  # here batches, groups, or advances nodes together; concurrency exists
                                  # only INSIDE a dispatched spawner skill. Compose EVERY brief from the
                                  # caller-side skeleton in ${CLAUDE_PLUGIN_ROOT}/snippets/dispatch-brief.md
                                  # (read it by path) plus the target agent's family delta - never inline
                                  # that file verbatim.
        ...
        - integrate         -> the land tail, ONCE PER REPO: § `integrate` node dispatch (the land tail)
                               below - pre-PR tail, THEN Existence precheck, THEN squash/push/open ONE PR
    ...
    contract = read_continuation_contract(node)              # SPAWNER node (skill invoked in `main`, owns its own workers you
                                                             # do not track): read its in-context AGGREGATE result inline. LEAF
                                                             # agent: read the contract from your own launch call's returned
    ...
    if node wrote source and returned a commit SHA:
        cherry_pick(sha, into = repos[node.repo].run_integration)   # Run start invariant 3 - saga + verify +
                                                             # checkpoint per references/run-integration.md
                                                             # § Run start procedure
    for nx in contract.next:                                 # SUGGEST -> CHAIN ; cross-workflow on_complete lands here too
        if nx.confidence >= 0.5 and not duplicate(nx) and within_budget:
            RUN.dynamic_nodes.append(materialize(nx))        # new READY node, depends_on = node; the tier
                                                             # function returns L2 for ANY dynamic node -
                                                             # human-gated, never auto-approved (GATE E-4)
        else:
...
Five checks, run before every dispatch: 1-4 are PURE FUNCTIONS of plan fields - no tool, no lookup;
5 compares a plan field against a runtime observable. Each either AGREES and the loop proceeds, or STOPS the run
BLOCKED naming the offending plan field. Never substitute, re-partition, re-order or re-plan.

1. **File-scope disjointness.** No two nodes' `files-in-scope` globs may overlap. On an overlap, STOP
   BLOCKED naming both node ids and the shared path, and **route back to `odoo-planning`** to
   re-partition. Never guess an owner and never create a worktree first.
2. **`repo: null` legality.** A `repo: null` node MUST be chat-only `inline` with no file outputs
   (§ Inputs). An illegal one STOPS the run BLOCKED naming the node and **routes back to
   `odoo-planning`**; it is never silently excluded from a repo's scope.
3. **Module presence.** Every node whose `approach` touches source MUST carry a non-empty
   `node.modules`. Never resolve those names yourself - the planner did when authoring, and the
   dispatched skill does again where the grounding already is. On a dispatch returning BLOCKED for
   an unresolvable module, do not retry and do not substitute: STOP the run BLOCKED naming the node
   and the module, and **route back to `odoo-planning`**.
4. **Verification floor.** When `node.approach == "odoo-instance"`, `node.modules` MUST cover the
   union of `modules` over that node's transitive `depends_on` closure. BELOW that floor: STOP
   BLOCKED naming the missing modules and **route back to `odoo-planning`** to re-scope. ABOVE it is
   the planner's deliberate widening (`skills/_shared/regression-scope.md`) - accept it and run it.
5. **Plan staleness.** The approved plan describes the tree as it stood at approval; before ANY
   worktree is created, verify the tree has not moved under it. STALE = repo `R`'s `base` tip is no
   longer the commit Run start forked `run-integration-<slug>` from (a bounded `git-toolkit:git-ops`
   read of `base`'s tip against that fork point). On STALE, do NOT re-derive the plan against the
   moved tree and do NOT proceed on the stale one - STOP BLOCKED naming `base`, both commits and the
   pending nodes, and **route back to `odoo-planning`** to re-plan against the current tree. Record
   both commits in `produced`; an unresolvable fork point is an open finding, never agreement.
...
Every gate this loop emits (`emit_human_gate`, the dynamic-node preview, the pre-PR tail's stage
gates, the terminal report) is chat-facing: write it in the USER'S language (translate the
...
gate_tier(node):
    ...
    # remaining kinds: skill | agent | workflow
    t = registry default_gate_tier(node.approach)        # generator/skill_tool_deps.json
...
**because you wrote the brief**: you set `MODE: fresh` + `PERSIST: ephemeral`, so the database is one
this run creates and destroys and nothing outside the run can observe it. That is the ONLY condition
that lowers a tier. A node dispatched to a skill owning its own instance policy (`odoo-i18n`,
`odoo-acceptance`) keeps its registry L2 - you did not write its brief, so you cannot prove its touch
is ephemeral. The ceiling reaches exactly one node kind today: `approach: odoo-instance`, via
§ Verification dispatch, the only place this driver composes an instance brief.

**Source-writing nodes** (targets source tree, not the `$ODOO_AI_HOME` state root) - **human gate
MUST be at the driver, before dispatch.** A spawner's worker subagent cannot pause for human input,
so the skill's internal Phase-0 gate is a safety-net, never the binding gate. Spawner skills writing
only under the `$ODOO_AI_HOME` state root (`odoo-code-review`, `odoo-ui-review`) need no extra
driver gate beyond registry tier.
...
- **Dynamic node** (materialized at runtime from `next[]` / `on_complete` - never in the
  approved plan): the function returns **L2**, so `--auto` cannot auto-pass it (GATE E-4
  all-dynamic-L2). Before dispatching you MUST emit a preview and **END YOUR TURN**:
  `Proposed / Files / OSM: backed | standalone` - spelling OSM out on first use: Odoo Semantic, the
  indexed Odoo source; backed = facts checked against it, standalone = not reachable, local files
  only - then `Gate: approve / refine: [feedback] / cancel`, the PLAN gate set
  (`${CLAUDE_PLUGIN_ROOT}/snippets/planning-gate-contract.md`).

**Defense-in-depth.** The tier reads the registry plus two facts you own (is this node in the
approved plan? did I write its brief against a fresh, ephemeral database?), so a hand-edited
`run.json` has nothing to lower - there is no stored tier to override. Hard rule 5 owns the one
irreversible landing this leaves.

## Verification dispatch

For a node whose `approach` is `odoo-instance`, compose the brief YOURSELF - never delegate it - from
the `${CLAUDE_PLUGIN_ROOT}/snippets/dispatch-brief.md` skeleton, filling every field of
...
is what makes the touch EPHEMERAL - the basis of the ceiling in § Gate-tier resolution. The node is
DONE only on a GREEN verdict read from the returned `instance-ops` block; `failed > 0` or
`tests-inconclusive` is FAILED, never DONE.

## integrate readiness

A PREDICATE evaluated at `pick_ready`. **`R` is the node's own `repo` field** (harness §8.3; one
`integrate` node per entry in `RUN.repos[]`).

**`integrate@R` is READY only when BOTH clauses hold.**
...
OTHER node in the repo must be DONE" deadlocks EVERY run. Another repo's nodes are outside this
scope, so a slow second repo never holds this PR hostage; a legal `repo: null` node is outside it too
(§ Plan agreement check 2 already proved that legality).
...
review / acceptance nodes when the plan under-specified `integrate.depends_on`.

## `integrate` node dispatch (the land tail)

Dispatched ONCE PER REPO (`integrate@R`), once § integrate readiness holds. In order:

1. **Pre-PR tail FIRST (MANDATORY, before the Existence precheck).** Drive every terminal stage that
   precedes `integrate` IN THE ORDER FIXED BY the Terminal stage order constant, then this driver's
   OWN pre-PR lint-class gate over repo `R`'s integration-branch aggregate diff. Never a per-run
   order, never a locally reordered one. The stages, that order, each stage's execution detail, the
   acceptance hand-off's `next` block, and the lint gate's dispatch fields: `run-integration.md`
   § Pre-PR tail > Terminal stage order - its ONE owner, not restated here.
2. **Existence precheck (MANDATORY, BEFORE any push or PR-open).** The land tail must be safe to run
   ...
   The two facts to read, the 3-case matrix and the rewrite-history gate: `run-integration.md`
   § Squash Tree-Identity Recipe.
3. **Land.** Invoke `git-toolkit:git-ops` from the main context to squash repo `R`'s run-integration
   branch, push it to the fork, and open ONE PR against `R`'s `base`, then materialize
   `next -> odoo-pr-monitoring` - the single outward merge gate, L2 from the registry, so the human
   approves the merge even under `--auto`. `odoo-coding` never pushes or opens a PR. No local merge
   into the principal, no auto-merge. Drive-to-done STOPS at "PR opened".
   **Exactly ONE PR per REPO per run** - N repos = N `integrate` nodes = N PRs, and no intermediate
   PR is ever opened.
