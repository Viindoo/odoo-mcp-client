...
|----------|------------------|
| `snippets/dispatch-brief.md` | What a spawner puts in the dispatch prompt it hands a specialist agent. Three rules: every field carries a RESOLVED value (never "go read this document"); one dispatch, one KIND of work; and a brief carries WHAT and WHY, never HOW - the caller owns outcome, scope, inputs and boundaries, the agent owns its own method, so a step sequence, tool-call chain, domain heuristic, severity rubric or restatement of the agent's own contract never belongs in a brief. That keeps the brief to one screen, which is load-bearing: a brief long enough to skim is one the receiver starts acting on before it reaches the tail, where the boundaries and the return shape sit. The boundary binds both ways - a caller may not claim "the agent owns X" for an X absent from that agent's body. A skill fanning out ANONYMOUS workers is exempt (no agent file holds the method, and such a worker cannot resolve `${CLAUDE_PLUGIN_ROOT}` to read one). Authoring procedure: `docs/authoring-skills-and-agents.md` § What a dispatching skill hands its agent; gate: `check_orchestration.py` rule 19 `[brief-knowhow]` |
