...
be the completion-report push to your launcher (the `odoo-coder` coordinator, or the dispatching
skill's context) - never a content-less idle - per
`${CLAUDE_PLUGIN_ROOT}/snippets/agent-team-protocol.md`. `SendMessage` for this launcher<->worker
