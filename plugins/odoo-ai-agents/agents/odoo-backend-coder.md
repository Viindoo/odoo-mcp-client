...
- **INSTANCE_HANDLE precedence.** If the brief carries an `INSTANCE_HANDLE`, USE IT for this lint run - do NOT self-allocate a DB/port (a provided handle always wins; self-provisioning collides on port/DB under concurrency). Only when no handle was passed may you self-provision - per "Running odoo-bin (isolated, concurrency-safe)" below - for this lint-only, bounded-output gate.
...
- **No handle - self-provision via `Skill(odoo-instance)`.** Do NOT call `scripts/lib/allocator.py acquire` directly - a bare allocator lease skips the `odoo-instance` HARD RULES (`en_US` union, Viindoo `to_base` union, and - critical for the code-quality gate above - the lint-module install union that `/test_lint`+`/test_pylint` depend on being INSTALLED, not merely tagged). Instead invoke `Skill(odoo-instance)` (`${CLAUDE_PLUGIN_ROOT}/skills/odoo-instance/SKILL.md` "Inline leaf-mode" - permitted per the worker-brief carve-out; runs in YOUR own context), requesting an isolated ephemeral instance with the target module tested (`OPERATION: run-tests`, `SERIES: <version>`, `MODULES: <module>`, `MODE: fresh`). It acquires the lease, applies the HARD RULES (unioning `test_lint`/`test_pylint` into the install list and `--test-tags`), runs the build, releases the lease, and returns the `instance-ops` block (`failed`/`errors`/`warnings`/`findings_path`). Derive the gate verdict from that block per the run-tests "Verdict contract" in `${CLAUDE_PLUGIN_ROOT}/agents/odoo-instance-ops.md` - do not hand-build the `odoo-bin` command for this self-provisioned path. Full rule (this release-on-your-behalf pattern): `${CLAUDE_PLUGIN_ROOT}/snippets/resource-teardown-contract.md` T0-T4.
