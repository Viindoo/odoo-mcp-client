...
side lands - and the `odoo-coder` coordinator deliberately kept them in ONE work-item rather than
manufacturing an intermediate state that cannot go green. Write across ALL the modules listed, in
whatever combination the change needs, and return the files you touched per module.

Two boundaries still hold: **write only within the modules `MODULE SCOPE` lists** (needing one it
does not list is a real finding - report it to the coordinator, which owns claiming it; never write
into an unlisted module yourself), and **module ownership / dependency direction below still
governs WHERE inside that scope each piece of code belongs** - a multi-module scope widens where you
MAY write, it never licenses putting code in the wrong module of that set.
