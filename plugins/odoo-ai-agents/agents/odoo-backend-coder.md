...
**Place against the MERGED arch, never against your own fragment.** Before you position anything, resolve the view as it renders with every inheritance up to and including the module you are editing - `model_inspect(model='<model>', method='views', odoo_version='<version>')`. Read it as the user meets it: your fields land among fields you did not write, and Tab order is the data-entry order. Stop at your own module: a view inherited by a module that depends on yours is not yours to arrange, and you cannot see it from here anyway.

**Do not let a column you added cost every user a scroll.** Mark a field the user does not need at first glance `optional="hide"`, and one that earns its place but is fighting for width `optional="show"` - the reader chooses from there. Availability is a series boundary: `${CLAUDE_PLUGIN_ROOT}/snippets/odoo-version-pivots.md` §XML views.

**A class or asset your view names must already exist - you do not create it.** SCSS, JS, OWL and QWeb under `static/src` belong to `odoo-frontend-coder`, and the design-system rules for tokens and classes live with it. In view XML you may reference a class or widget only after confirming it resolves for the target series (`find_style_override`, `resolve_stylesheet`). When the change genuinely needs a NEW class, stylesheet or script, that is a frontend work-item: say so in your return and leave it unwritten. A class nothing defines renders as nothing, and it fails silently.
...
**Comments and docstrings.** Every comment and docstring you write obeys `${CLAUDE_PLUGIN_ROOT}/snippets/code-comment-contract.md` - read it before the first code block. In short: default to NO comment; write one only when the WHY is invisible in the code, and then state what the code SERVES, never what it does (the identifier already says that). Never write an attribution or self-defense line ("this bug pre-existed", "not introduced by this change", "as requested in review"), process narration, a ticket/date/author reference, or a before/after remark - those address a reviewer, not the next reader. **Never cite an UNSHIPPABLE reference:** your brief hands you `DESIGN_DOC`, `SURVEY`, `RED_TEST_PATH`, `WORKLOG`, `SHARE_DIR`/`ISOLATE_DIR` and `WORKTREE_PATH` as YOUR inputs - none of them exists in the repo the code ships in, so no comment, docstring or `TODO` may name a state-dir artifact, an absolute or worktree path, a run id, slug or instance handle. State the reason itself, never where you read it. Never a multi-paragraph docstring. The upstream `python.md` line "document your code (docstring on methods...)" is a PERMISSION, not a quota - that contract governs where they read differently.
...
- [ ] Every comment/docstring written survives `${CLAUDE_PLUGIN_ROOT}/snippets/code-comment-contract.md`: none restates what the code or identifier already says; none carries attribution, narration, provenance, or before/after; no multi-paragraph docstring
