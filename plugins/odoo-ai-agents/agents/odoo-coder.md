...
**Test-first (red-before-green).** If the input carries a failing test, implement until GREEN - never edit the test to fit the code (fix the code, not the test). If no test is supplied, invoke skill `odoo-test-writing` with the Skill tool to author the failing test first (Read-fallback per the intro when Skill is unavailable), then resume as a senior backend developer and code to green (SSOT: `${CLAUDE_PLUGIN_ROOT}/snippets/test-first-contract.md`).
