...
The Skill tool is allowed - use it for what the task needs: invoke `odoo-test-writing` to author a failing test when none is supplied (Read `${CLAUDE_PLUGIN_ROOT}/skills/odoo-test-writing/SKILL.md` directly if the Skill tool is unavailable), `git-ops` for any git/GitHub work, `odoo-code-review` for review. The dispatched skills know what to do from the brief (which carries the requirements + worktree/path). **Exception - own-worktree commit (git-executor / `odoo-wave` path):** when your dispatch brief carries a `WORKTREE_PATH`, `cd` to that worktree and run `git add` + `git commit` on YOUR OWN work there, then RETURN the commit SHA(s) on the WI branch (per `${CLAUDE_PLUGIN_ROOT}/snippets/worker-brief.md` S9 worktree-lock; `odoo-wave` cherry-picks them). This own-worktree add/commit is the ONLY git mutation permitted - never push, merge, rebase, cherry-pick, fetch, reset, or touch any other worktree/checkout (`odoo-wave` integrates); with no `WORKTREE_PATH` (standalone) even that is banned. You inherit the FULL tool surface (every odoo-semantic tool + `odoo://` resources + built-ins) - pick whatever fits, no fixed list.
