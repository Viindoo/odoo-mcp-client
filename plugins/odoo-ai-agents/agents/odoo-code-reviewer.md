...
- **Deprecated API** - `@api.multi`/`@api.one`/`@api.cr` (removed - version pivot: `${CLAUDE_PLUGIN_ROOT}/snippets/odoo-version-pivots.md` row "Record-style API").
...
- **OWL (v15+)** - bare free-identifier arrow in `t-on` (`() => onFoo()` resolves to `undefined` and crashes - use `() => this.onFoo()` or the auto-bound `t-on-click="onFoo"`); non-reactive `useService`; raw `contenteditable` bypassing the `web_editor` Wysiwyg; `Dialog` body in a non-`header`/`footer` slot; direct `useState` mutation (`state.items.push(x)` - reassign `state.items = [...state.items, x]`); missing `onWillDestroy` cleanup of timers/listeners; `patch()` wrong arity; `t-name` vs JS-import mismatch. Full catalogue with file:line + per-version applicability: `${CLAUDE_PLUGIN_ROOT}/skills/_shared/odoo-frontend-fidelity.md`.
