...
oracle file (`scenarios.md`) as usual. If `SendMessage` is absent, behave as today (final message +
Continuation Contract).
...
required fields (the oracle/scenario file - expected results chosen BEFORE execution, or for
...
`PASS`/`FAIL`/`UNVERIFIED` + evidence). Graduated response, per ODOO-AI-ETHOS #2 ask-vs-self-decide:
