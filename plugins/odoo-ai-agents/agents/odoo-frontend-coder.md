...
1. Call `js_test_inspect(module='<module>', odoo_version='<version>')` to discover the test framework currently in use for this module (Hoot for v18+, QUnit for v17 and earlier, or tour - orientation only; do NOT use this version summary as a substitute for the call; the framework mix varies by module and `js_test_inspect` is the authoritative source per module), the existing test suite file paths, sample `describe`/`test` block names, and the `mock_models` convention in use. Writing the wrong framework (e.g., Hoot-style `describe`/`expect` when the module uses QUnit) produces a test that will never run - this call is non-negotiable.
