...
- **Tier 2 - version:** Read `<SHARE_DIR>/context.md` for `odoo_version` (resolve `<SHARE_DIR>`/`<ISOLATE_DIR>` once per `${CLAUDE_PLUGIN_ROOT}/snippets/state-root-resolution.md`; substitute the captured absolute path - never write the placeholder or a bare `.odoo-ai/` into a Read/Write/Edit); if absent, `find . -maxdepth 4 -name __manifest__.py | head -1` then Read (first two dotted components = Odoo version).
...
1. Read `<SHARE_DIR>/context.md` if present for `odoo_version` and `profile`; fall back to user-stated version, then manifest discovery (`${CLAUDE_PLUGIN_ROOT}/snippets/context-bootstrap.md`).
